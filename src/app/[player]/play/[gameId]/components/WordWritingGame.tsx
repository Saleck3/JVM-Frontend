...
type Props = {
	...
	preselectedLetters: { letter: string; index: number }[];
	imgSrc: string;
	onWrongAnswer: () => void;
...
const WordWritingGame = (props: Props) => {
	...
	const handleCheck = () => {
		...
		if (correctAnswer.toLocaleLowerCase() === inputValue.toLocaleLowerCase()) {
			setIsCorrect(true);
			playCorrectSound();
		} else {
			setIsIncorrect(true);
	...
	const inputStyle = {
		fontSize: '3rem',
		border: 'none',
		width: `${correctAnswer.length * (letterWidth + gapWidth)}ch`,
		background:
